...
/**
 * Renders a searchable, selectable card grid inside a dialog HTML field.
 *
 * @param {object}   dialog        - frappe.ui.Dialog instance
 * @param {string}   html_field    - fieldname of the HTML field to render into
 * @param {string}   card_class    - CSS class for individual card checkboxes
 * @param {string}   count_id      - DOM id for the "X selected" counter span
 * @param {string}   search_id     - DOM id for the search input
...
function render_card_grid(dialog, html_field, card_class, count_id, search_id, items) {
    // Store items on dialog for re-render on search
    ...
    // Select All / Deselect All  (acts on currently visible cards)
